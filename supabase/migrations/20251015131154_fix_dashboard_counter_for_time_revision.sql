-- Migration: Fix dashboard counter for time revision requests
-- Description: Include 'specialist_time_revision_requested' status in 'in_analysis' counter
-- When specialist requests time revision, the quote should still be considered "in analysis"
-- from the partner's perspective, as it's waiting for partner action.
...
BEGIN
    ...
    SELECT json_build_object(
        ...
        -- ✅ CORREÇÃO: Incluir 'specialist_time_revision_requested' pois ainda está em análise
...
CORREÇÃO APLICADA (15/10/2025):
- Incluído status ''specialist_time_revision_requested'' no contador ''in_analysis''
- Orçamentos com solicitação de revisão de prazo continuam sendo considerados "em análise"
- Contador não decrementa mais quando especialista solicita revisão
';