...
DROP FUNCTION IF EXISTS get_client_vehicles_paginated(uuid, integer, integer, text, text[], text[]);

-- Recreate the function with the new p_today_date parameter
CREATE OR REPLACE FUNCTION get_client_vehicles_paginated(
    ...
    p_date_filter text[],
    p_today_date date -- New parameter
)
RETURNS json AS $$
DECLARE
    v_offset integer;
...
BEGIN
    ...
    WITH filtered_vehicles AS (
        ...
        WHERE
            ...
            -- Date filter logic now uses p_today_date
            AND (
