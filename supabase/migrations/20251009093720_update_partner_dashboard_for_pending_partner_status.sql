-- Atualizar função get_partner_dashboard_data para suportar o novo status 'pending_partner'
-- Problema: RPC estava verificando status IS NULL, mas agora usamos 'pending_partner'
-- Solução: incluir 'pending_partner' nas verificações de quotes pendentes
...
BEGIN
    ...
    SELECT json_build_object(
        'total', COUNT(*),
        -- PENDENTE: Quotes com status 'pending_partner' (aguardando parceiro preencher)
...
- pending_quotes: quotes que precisam de ação do parceiro (status pending_partner ou pending)
- in_progress_services: quotes aprovadas aguardando execução

IMPORTANTE:
- PENDENTE: status = pending_partner (parceiro precisa preencher)
