-- Migration: update_partner_save_checklist_with_partner_service
-- Purpose: Update function to include partner_service column when creating vehicle history
...
BEGIN
  IF EXISTS (
    ...
    DROP FUNCTION public.partner_save_checklist_update_vehicle_status(uuid, uuid);
  END IF;
END $$;
...
BEGIN
  ...
  -- Criar entrada no histórico do veículo (agora com partner_service)
...
IS 'Updates vehicle status to "Fase Orçamentaria" and creates vehicle_history entry with partner_service when partner saves checklist. SECURITY DEFINER with access check. Auto-detects service category from pending quote.';
