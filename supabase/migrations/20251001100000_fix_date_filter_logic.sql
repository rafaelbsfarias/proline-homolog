-- Drop the previous function signature to avoid overloading
DROP FUNCTION IF EXISTS get_client_vehicles_paginated(uuid, integer, integer, text, text[], text[], date);

-- Recreate the function with the corrected date filter logic
            ...
            AND (
                p_date_filter IS NULL OR cardinality(p_date_filter) = 0 OR (
                    estimated_arrival_date IS NOT NULL AND (
                        ('Atrasado' = ANY(p_date_filter) AND estimated_arrival_date::date <= p_today_date AND status NOT IN ('Análise Finalizada', 'Cancelado')) OR
                        ('Próximo (5 dias)' = ANY(p_date_filter) AND estimated_arrival_date::date > p_today_date AND estimated_arrival_date::date <= (p_today_date + interval '5 days') AND status NOT IN ('Análise Finalizada', 'Cancelado')) OR
                        ('No Prazo' = ANY(p_date_filter) AND estimated_arrival_date::date > (p_today_date + interval '5 days') AND status NOT IN ('Análise Finalizada', 'Cancelado'))
                    )
                )
            )
    )
