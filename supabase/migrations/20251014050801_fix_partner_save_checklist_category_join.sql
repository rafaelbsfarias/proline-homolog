-- Migration: fix_partner_save_checklist_category_join
-- Purpose: Fix the partner_save_checklist_update_vehicle_status function to use correct join path
-- Issue: Function was trying to access so.inspection_service_id which doesn't exist
-- Solution: Use so.category_id directly since service_orders now has category_id column
...
BEGIN
  ...
  -- CORRIGIDO: Usar category_id diretamente de service_orders
...
IS 'Updates vehicle status to "Fase Orçamentaria" and creates vehicle_history entry with partner_service when partner saves checklist. SECURITY DEFINER with access check. Auto-detects service category from pending quote. IDEMPOTENT - checks if entry exists before inserting. FIXED: Uses category_id directly from service_orders.';
