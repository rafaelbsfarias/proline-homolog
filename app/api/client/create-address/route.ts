...
const logger: ILogger = getLogger('CreateAddressAPI');
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    logger.info('create-address:request_received', {
      requestId,
      path,
      method,
      hasBody: !!body,
    ...
    if (!authHeader?.startsWith('Bearer ')) {
      logger.warn('create-address:missing_auth', { requestId, path, method });
    ...
    if (userError || !user) {
      logger.warn('create-address:unauthenticated', { requestId, path, method, userError: userError?.message });
    ...
    logger.info('create-address:success', { requestId, path, method, userId: user.id, addressId: inserted?.id });
    return NextResponse.json({ success: true, message: 'Endereço criado com sucesso!', address: inserted });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);

    // Detect Supabase schema cache or migration mismatch issues related to the new columns
    const schemaOutOfSync = /schema cache|is_collect_point|is_main_address/i.test(message);
    if (schemaOutOfSync) {
      logger.error('create-address:schema_out_of_sync', {
        requestId,
        path,
        method,
    ...
    if (error instanceof ValidationError) {
      logger.warn('create-address:validation_error', { requestId, path, method, code: 'INVALID_INPUT', error: error.message });
      return NextResponse.json({ error: error.message, code: 'INVALID_INPUT' }, { status: 400 });
    }
    if (error instanceof NotFoundError) {
      logger.warn('create-address:access_denied', { requestId, path, method, code: 'ACCESS_DENIED', error: error.message });
      return NextResponse.json({ error: error.message, code: 'ACCESS_DENIED' }, { status: 403 });
    }
    if (error instanceof DatabaseError) {
      logger.error('create-address:db_error', { requestId, path, method, code: 'DATABASE_ERROR', error: error.message });
      return NextResponse.json({ error: error.message, code: 'DATABASE_ERROR' }, { status: 500 });
    }
    if (error instanceof AppError) {
      logger.error('create-address:app_error', { requestId, path, method, code: 'APP_ERROR', error: error.message });
      return NextResponse.json({ error: error.message, code: 'APP_ERROR' }, { status: error.statusCode });
    }
    logger.error('create-address:unhandled_error', { requestId, path, method, error: message });
