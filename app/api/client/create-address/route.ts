...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (userError || !user) {
      logger.warn('create-address:unauthenticated', {
        requestId,
        path,
        method,
    ...
    logger.info('create-address:success', {
      requestId,
      path,
      method,
      userId: user.id,
    ...
    if (schemaOutOfSync) {
      ...
      return NextResponse.json(
        {
          error:
            'Estrutura do banco desatualizada. Aplique as migrations e atualize o cache do schema.',
          code: 'SCHEMA_OUT_OF_SYNC',
    ...
    if (error instanceof ValidationError) {
      logger.warn('create-address:validation_error', {
        requestId,
        path,
        method,
        code: 'INVALID_INPUT',
    ...
    if (error instanceof NotFoundError) {
      logger.warn('create-address:access_denied', {
        requestId,
        path,
        method,
        code: 'ACCESS_DENIED',
    ...
    if (error instanceof DatabaseError) {
      logger.error('create-address:db_error', {
        requestId,
        path,
        method,
        code: 'DATABASE_ERROR',
    ...
    if (error instanceof AppError) {
      logger.error('create-address:app_error', {
        requestId,
        path,
        method,
        code: 'APP_ERROR',
