...
const logger = getLogger('api:client:reject-collection');
...
export const POST = withClientAuth(async (req: AuthenticatedRequest) => {
  try {
    ...
    const { error: vehicleError } = await supabase
      .from('vehicles')
      .update({ status: 'PONTO DE COLETA SELECIONADO' }) // Revert status
    ...
    if (collectionError) {
      ...
      return NextResponse.json({ success: false, error: 'Erro ao rejeitar coleta' }, { status: 500 });
    }
