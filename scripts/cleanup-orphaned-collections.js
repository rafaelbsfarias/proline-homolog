...
class OrphanedCollectionsCleaner {
  ...
  async deleteOrphanedCollection(collectionId, dryRun = true) {
    try {
      ...
      await this.client.query('DELETE FROM collection_history WHERE collection_id = $1', [
        collectionId,
      ]);

      // Depois, deletar a collection
      const result = await this.client.query('DELETE FROM vehicle_collections WHERE id = $1', [
        collectionId,
      ]);
