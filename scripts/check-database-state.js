...
const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://qcmlwbnfnxdlglcinkho.supabase.co';
...
class DatabaseTester {
  ...
  async testCollectionsState() {
    ...
    try {
      ...
      if (total > 0) {
        ...
        const statusStats = collections.reduce((acc, coll) => {
        ...
        const orphanedCollections = collections.filter(coll => !coll.vehicles || coll.vehicles.length === 0);
        if (orphanedCollections.length > 0) {
          console.log(`\n⚠️  Collections órfãs encontradas: ${orphanedCollections.length}`);
          this.report.issues.push(`${orphanedCollections.length} collections sem veículos associados`);
          orphanedCollections.forEach(coll => {
            console.log(`   ID: ${coll.id.slice(0, 8)}... | Status: ${coll.status} | Data: ${coll.collection_date}`);
          });
        }
        ...
        collections.slice(0, 5).forEach((coll, index) => {
          ...
          console.log(`   ${index + 1}. ${coll.collection_date} | ${coll.status} | ${vehicleCount} veículo(s) | ID: ${coll.id.slice(0, 8)}...`);
        });
      }
  ...
  async testVehiclesState() {
    ...
    try {
      ...
      if (total > 0) {
        // Estatísticas por status
        const statusStats = vehicles.reduce((acc, veh) => {
        ...
        const vehiclesWithCollections = vehicles.filter(v => v.collection_id);
        const vehiclesWithoutCollections = vehicles.filter(v => !v.collection_id);
        ...
        const inconsistentVehicles = vehicles.filter(v =>
          v.collection_id &&
        ...
        if (inconsistentVehicles.length > 0) {
          ...
          this.report.issues.push(`${inconsistentVehicles.length} veículos com status inconsistente entre vehicle e collection`);
        }

        // Veículos com solicitação de mudança de data
        const dateChangeRequests = vehicles.filter(v => v.status === 'SOLICITAÇÃO DE MUDANÇA DE DATA');
  ...
  async testHistoryState() {
    ...
    try {
      ...
      if (total > 0) {
        ...
        console.log(`💰 Receita total histórica: R$ ${totalRevenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`);
        ...
        history.slice(0, 5).forEach((hist, index) => {
          console.log(`   ${index + 1}. ${hist.collection_date} | ${hist.vehicle_count} veículos | R$ ${(hist.total_amount || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`);
        });
      }
  ...
  async testTriggersAndFunctions() {
    ...
    console.log('ℹ️  Verificação limitada via Supabase - para análise completa use PostgreSQL direto');

    this.report.details.triggers = {
      note: 'Verificação limitada via Supabase. Para análise completa, use PostgreSQL direto.',
      expectedTriggers: [
        'trigger_create_collection_history',
        'trg_vehicle_collections_set_ts'
      ]
    };
  }
    ...
    try {
      ...
      if (total > 0) {
        ...
        logs.forEach((log, index) => {
          console.log(`   ${index + 1}. [${log.timestamp}] ${log.action} ${log.resource_type}: ${log.details?.slice(0, 50)}...`);
        });
      } else {
    ...
    try {
      ...
      if (!vehError && vehiclesWithCollections) {
        ...
        if (!collError && existingCollections) {
          ...
          const orphanedVehicles = vehiclesWithCollections.filter(v => !existingIds.has(v.collection_id));
  ...
  generateReport() {
    ...
    if (summary.orphanedCollections > 0) {
      this.report.recommendations.push('Limpar collections órfãs que não estão associadas a veículos');
    }
    ...
    console.log(`💰 Receita total: R$ ${summary.totalRevenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`);
