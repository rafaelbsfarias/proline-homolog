    ...
    try {
      ...
      if (total > 0) {
        ...
        collections.slice(0, 5).forEach((coll, index) => {
          console.log(`   ${index + 1}. ${coll.collection_date} | ${coll.status} | ${coll.vehicle_count} veículo(s) | ID: ${coll.id.slice(0, 8)}...`);
        });
      }
    ...
    try {
      ...
      if (total > 0) {
        ...
        history.slice(0, 5).forEach((hist, index) => {
          console.log(`   ${index + 1}. ${hist.collection_date} | ${hist.vehicle_count} veículos | R$ ${(parseFloat(hist.total_amount) || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`);
        });
      }
    ...
    try {
      ...
      if (dateChangeVehicles.length > 0) {
        ...
        dateChangeVehicles.forEach(v => {
          console.log(`   ${v.plate}: ${v.estimated_arrival_date} (Collection: ${v.collection_id?.slice(0, 8)}...)`);
        });
      }
