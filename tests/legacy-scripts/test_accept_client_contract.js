...
async function testAcceptClientContract() {
  ...
  if (clientError || !clientProfile || clientProfile.length === 0) {
    console.error(
      'Erro: Nenhum perfil de cliente encontrado. Por favor, crie um cliente primeiro.'
    );
    process.exit(1);
  }

  const p_client_id = clientProfile[0].id;
  ...
  try {
    ...
    if (error) {
      ...
      console.log(
        'Verifique a tabela client_contract_acceptance no Supabase para confirmar a inserção/atualização.'
      );
    }
  } catch (error) {
